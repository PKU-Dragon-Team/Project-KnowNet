...
class ScopusFulltextSpider(object):
    ...
    def execute(self) -> tg.Text:
        ...
        if succ:
            ...
            with open(self.output_path + self.filename, 'w', encoding='utf-8') as f:
                f.write(str(self.doc._data))
            return self.output_path + self.filename
        err_msg = self.doc._err_msg
        self._logger.error(
            'ScopusFulltextSpider, getting doi = ' + self.doi + 'failed. Exception: ' + str(err_msg)
        )
        return None
