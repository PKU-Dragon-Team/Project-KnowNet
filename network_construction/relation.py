...
def relation_extraction_text(source, document, node, relation, database):
    ...
    if node == "noun" and relation == "co":
        for a in text:
            all_text = a['text']
            relation = algorithm.extract_relation(all_text)
            for r in relation:
                node1_key = "word_" + r[0]
                node2_key = "word_" + r[1]
            ...
            if len(node1_author) > 1:
                for i in range(0, len(node1_author)-1):
                    for j in range(i+1, len(node1_author)):
                        ...
                        relation_struct = db.search_author_relation(node1, node2, database)
                        if relation_struct:
...
if __name__ == '__main__':
    relation_extraction_author("ScienceDirectDataSource","1-10","all","knowledge2")
    #经过测试可用！
